...
public:
  ...
  void begin();
protected:
  int bits, sclk, miso, mosi, ss, spiCLK, value;
  SPIClass *mcp;
  void config();
  void SPIWrite(SPIClass *spi, uint8_t addr, uint32_t REG_CFG);
  uint32_t SPIRead(SPIClass *spi, uint8_t addr, _u16data_t &COMM_CRC, uint64_t &CALC_CRC);
